...
use crate::error::Error::{DecryptionFailed, EncryptionFailed, InvalidFormat, KeyDerivationFailed};
...
fn derive_key(password: &str, salt: &[u8]) -> Result<[u8; 16], Error> {
    ...
    let salt_string = SaltString::encode_b64(salt).map_err(|_| KeyDerivationFailed)?;
    let password_hash = argon2
        .hash_password(password.as_bytes(), &salt_string)
        .map_err(|_| KeyDerivationFailed)?;
    let hash_bytes = password_hash
        .hash
        .ok_or(KeyDerivationFailed)?;
...
pub fn decrypt(enc_key: &str, encrypted_data: &str) -> Result<String, Error> {
    ...
    String::from_utf8(plaintext).map_err(|_| InvalidFormat)
}
